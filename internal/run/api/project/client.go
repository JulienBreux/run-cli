...
func (c *GCPClient) getClient(ctx context.Context) (ProjectsClientWrapper, error) {
	...
	creds, err := client.FindDefaultCredentials(initCtx, resourcemanager.DefaultAuthScopes()...)
	if err != nil {
	...
	cClient, err := createProjectsClient(initCtx, option.WithCredentials(creds))
	if err != nil {
		return nil, err
	}
