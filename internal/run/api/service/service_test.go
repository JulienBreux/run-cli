...
func BenchmarkList_AllRegions_Comparison(b *testing.B) {
	originalClient := apiClient
	defer func() { apiClient = originalClient }()
	...
	b.Run("Original_Mutex_Approach", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			var (
				mu       sync.Mutex
			...
			for _, r := range api_region.List() {
				wg.Add(1)
				go func(region string) {
					defer wg.Done()
					if s, err := List("p", region); err == nil {
						mu.Lock()
	...
	b.Run("Optimized_LockFree_MapReduce", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			_, _ = List("p", api_region.ALL)
		}
	})
}
